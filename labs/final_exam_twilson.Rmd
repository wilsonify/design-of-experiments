...
Let y be the repsose in light, b be Day (the blocking factor), and tau be the type treatment.
...
This model assumes that the error term is normally distirbuted with mean of zero and homogenous variance of $\sigma^2$ across levels of the treatment and block.
...
```{r}
mean(graphite_data$light)
```

```{r}
graphite_model <- aov(formula = light ~ day+type, data=graphite_data)
coef(graphite_model) %>% kable()
```
