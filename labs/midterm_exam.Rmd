...
```{r}
brands <- c("b1","b2","b3")
data <- data.frame(brand=rep(brands,5),
...
data <- data.frame(coating = rep(coatings,each=4),
...
data <- expand.grid(jet_efflux_velocity = jet_efflux_velocities,nozzle=nozzles)
data <- cbind(data,shape_factor=c(0.78,0.80,0.81,0.75,0.77,0.78,
...
data <- expand.grid(cu_content=rep(cu_contents,2),temperature=temperatures)
data <- cbind(data,warping=c(17,16,24,28,
                             20,21,22,27,
                             12,18,17,27,
                             09,13,12,31,
                             16,18,25,30,
