...
```{r}
epitaxial_model <- lm(formula = thickness~flow_rate*depo_time,
                       data=epitaxial_data)
...
```{r}
anova(epitaxial_model)
```

deposition time is significant
while flow rate and the interaction are not siginifcantly different from coefficient=0
...
```
There is 1 very serious outlier.

Find the cook distance. Take out the outlier(s). Rebuilt the rsm model on new data.

```{r}
is_not_outlier <- cooks.distance(epitaxial_model)<0.025
...
```{r}
cracking_model <- lm(formula=crack_length~pouring_temperature*titanium_content*heat_treatment_method*grain_refiners, data=cracking_data)
#summary(cracking_model)
```

```{r}
cracking_model <- aov(formula=crack_length~pouring_temperature*titanium_content*heat_treatment_method*grain_refiners, data=cracking_data)
#summary(cracking_model)
```
There are significant interactions between pouring_temperature,titanium_content, and heat_treatment_method 


```{r}
LenthPlot(cracking_model, alpha = 0.05, plt =TRUE, limits = TRUE)
```
